...
async def get_face_accounts(api_key: str) -> tuple[bool, list[str], str]:
    ...
    # Build original-case username list and try bulk format first
    face_usernames_orig: list[str] = []
    for acc in all_accounts:
        ...
        is_face = u.lower() in all_face_names or "status:face" in tag_strs
        if not is_face:
            continue
        face_usernames_orig.append(u)
        line = _format_for_zp(acc)
        if line:
            formatted.append(line)
    ...
    # Strategy 3: bulk endpoint had no cookies — fetch via POST /api/accounts/credentials
    if face_usernames_orig:
        creds = await get_accounts_credentials(api_key, face_usernames_orig)
        for acc in creds:
            line = _format_for_zp(acc)
            if line:
                formatted.append(line)
...
async def get_accounts_credentials(api_key: str, usernames: list[str]) -> list[dict]:
    """Fetch credentials (including cookies) for a list of usernames.
    Uses POST /api/accounts/credentials. Returns list of account dicts."""
    if not usernames:
        return []
    CHUNK = 100
    result: list[dict] = []
    for i in range(0, len(usernames), CHUNK):
        chunk = usernames[i:i + CHUNK]
        ok, data, _ = await _post(api_key, "/api/accounts/credentials", {"usernames": chunk})
        if not ok or not data:
            continue
        if isinstance(data, list):
            result.extend(data)
        elif isinstance(data, dict):
            result.extend(data.get("accounts") or [])
    return result
