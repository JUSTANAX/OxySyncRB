...
def get_devicetrim_config(user_id: int) -> dict | None:
    c = _get_client()
    result = c.table("devicetrim_config").select("*").eq("user_id", user_id).execute()
    if not result.data:
        return None
    row = result.data[0]
    return {
        "auto_enabled":   bool(row.get("auto_enabled", 0)),
        "interval_hours": float(row.get("interval_hours") or 1.0),
        "max_per_device": int(row.get("max_per_device") or 300),
        "last_run_at":    row.get("last_run_at"),
    }


def _upsert_devicetrim(user_id: int, fields: dict):
    c = _get_client()
    result = c.table("devicetrim_config").update(fields).eq("user_id", user_id).execute()
    if not result.data:
        try:
            c.table("devicetrim_config").insert({"user_id": user_id, **fields}).execute()
        except Exception:
            pass


def toggle_devicetrim_auto(user_id: int) -> bool:
    c = _get_client()
    result = c.table("devicetrim_config").select("auto_enabled").eq("user_id", user_id).execute()
    current = bool(result.data[0]["auto_enabled"]) if result.data else False
    new_val = not current
    _upsert_devicetrim(user_id, {"auto_enabled": int(new_val)})
    return new_val


def save_devicetrim_interval(user_id: int, hours: float):
    _upsert_devicetrim(user_id, {"interval_hours": hours})


def save_devicetrim_max(user_id: int, max_per_device: int):
    _upsert_devicetrim(user_id, {"max_per_device": max_per_device})


def set_devicetrim_last_run(user_id: int):
    _upsert_devicetrim(user_id, {"last_run_at": _now_iso()})


def get_users_due_for_devicetrim() -> list[tuple[int, str, int]]:
    c = _get_client()
    records = c.table("devicetrim_config").select("user_id, interval_hours, last_run_at, max_per_device") \
        .eq("auto_enabled", 1).execute().data
    if not records:
        return []
    now = datetime.utcnow()
    ...
    panels_map = {p["user_id"]: p["api_key"] for p in panels}
    return [
        (r["user_id"], panels_map[r["user_id"]], int(r.get("max_per_device") or 300))
        for r in due if r["user_id"] in panels_map
    ]
...
def get_autotrade_config(user_id: int) -> dict | None:
    c = _get_client()
    result = c.table("autotrade_config").select("*").eq("user_id", user_id).execute()
    if not result.data:
        return None
    row = result.data[0]
    return {
...
def _upsert_autotrade(user_id: int, fields: dict):
    c = _get_client()
    result = c.table("autotrade_config").update(fields).eq("user_id", user_id).execute()
    if not result.data:
        try:
            c.table("autotrade_config").insert({"user_id": user_id, **fields}).execute()
        except Exception:
            pass


def save_autotrade_config_id(user_id: int, config_id: int):
    _upsert_autotrade(user_id, {"config_id": config_id})


def set_autotrade_running(user_id: int, running: bool):
    _upsert_autotrade(user_id, {"running": int(running)})


def set_autotrade_started_at(user_id: int):
    _upsert_autotrade(user_id, {"started_at": _now_iso()})


def get_autotrade_accounts(user_id: int) -> list[str]:
    c = _get_client()
    result = c.table("autotrade_accounts").select("username").eq("user_id", user_id).order("id").execute()
    return [r["username"] for r in result.data]


def add_autotrade_account(user_id: int, username: str) -> bool:
    c = _get_client()
    existing = c.table("autotrade_accounts").select("id").eq("user_id", user_id).eq("username", username).execute()
    if existing.data:
        return False
    c.table("autotrade_accounts").insert({"user_id": user_id, "username": username}).execute()
    return True


def add_autotrade_accounts_bulk(user_id: int, usernames: list[str]) -> tuple[int, int]:
    c = _get_client()
    existing_rows = c.table("autotrade_accounts").select("username").eq("user_id", user_id).execute()
    existing_set  = {r["username"].lower() for r in existing_rows.data}
    new = [u for u in usernames if u and u.lower() not in existing_set]
    if new:
        c.table("autotrade_accounts").insert([{"user_id": user_id, "username": u} for u in new]).execute()
    return len(new), len(usernames) - len(new)


def clear_autotrade_accounts(user_id: int):
    c = _get_client()
    c.table("autotrade_accounts").delete().eq("user_id", user_id).execute()
